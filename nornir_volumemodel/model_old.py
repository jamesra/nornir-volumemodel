...
class VolumeManager():
    ...
    def Load(cls, VolumePath, Create=False, UseCache=True):
        ...
        return VolumeRoot
        # return cls.py(VolumeData, Filename)
...
class XElementWrapper(ElementTree.Element):
    ...
    @classmethod
    def LoadFromXML(cls):
        raise NotImplemented("Subclasses must implement LoadFromXML")
    ...
    def wrap(cls, dictElement):
        ...
        obj = cls()
        obj.update(dictElement)

        # dictElement.__class__ = cls

        if not 'CreationDate' in dictElement.attrib:
            cls.logger.info("Populating missing CreationDate attribute " + obj.ToElementString())
            obj.attrib['CreationDate'] = obj.__GetCreationTimeString__()

        if(isinstance(obj, XContainerElementWrapper)):
            if(not 'Path' in obj.attrib):
                print(obj.ToElementString() + " no path attribute but being set as container")
            assert('Path' in obj.attrib)
